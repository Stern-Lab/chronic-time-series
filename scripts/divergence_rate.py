...
def label_non_coding_regions(data, non_coding_regions):
    ...
    :param data: Input DataFrame with mutation positions.
    :param non_coding_regions: List of tuples where each tuple defines the start and end of a non-coding region.
    :return:  Returns the DataFrame with an updated 'type' column.
    """
...
def label_syn_non_syn_reversion(data):
    ...
    :param data: The input DataFrame with 'protein_type' and 'mutation_type' columns.
    :return: The DataFrame with an updated 'type' column.
    """
    types = []
...
def calculate_divergence(data, mutation_type, mutation_percentage, genome_length):
    ...
    :return: Returns the normalized divergence values for the given mutation type.
    """
...
def calculate_divergence_by_gene(data, mutation_type):
    """
    Calculate the divergence values for a given mutation type aggregated by gene.
    :param data: Input DataFrame with 'type', 'final_freq', 'protein', and 'timepoint' columns.
    :param mutation_type: The mutation type ('non-synonymous' or 'synonymous').
    :return: Returns sum of frequencies of each mutation type (syn/non-syn) aggregated by gene.
    """
...
def add_missing_timepoints(data, timepoints):
   ...
   # Iterate over each patient
    for patient, patient_data in data.groupby('patient_id'):
        ...
        # Filter the timepoints to include only those relevant for the patient id (pid)
        ...
        if os.path.isdir(patient_id_path):
            ...
            for timepoint_folder in os.listdir(patient_id_path):
                ...
                if os.path.isdir(timepoint_path):
                    ...
                    # Count the number of non-synonymous and synonymous mutations (was used here just for sanity check)
...
def plot_normalized_divergence_rates_per_patient_with_regression(final_df, genome_length_without_non_coding,
    ...
    # Arguments to set a detailed font size for the plot
    SMALL_SIZE = 10
    MEDIUM_SIZE = 15
    BIGGER_SIZE = 20
    TITLE_SIZE = 25
...
def plot_normalized_divergence_rates_per_patient_with_regression_P5_75_mod(final_df, genome_length_without_non_coding,
                                                    mutation_percentage_non_synonymous, mutation_percentage_synonymous):
    """
    This function was used just to check if time-point 75 will have an equal value as time-point 68 for P5,
    what will be the effect on the regression line.
    :param final_df:
...
def plot_divergence_by_gene(final_df):
    """
    Plot the divergence rates aggregated by gene.
    :param final_df:The final DataFrame with divergence rates.
    :return:
    """
...
def plot_divergence_by_gene_per_patient(final_df):
    """
    Plot the divergence rates aggregated by gene for each patient.
    :param final_df:
    :return:
    """
    # Drop rows with protein = 'NA'
...
def plot_non_synonymous_divergence_all_patients(final_df, genome_length_without_non_coding, mutation_percentage_non_synonymous):
    """
    Plot divergence rates for non-synonymous mutations over time for all patients in a single scatter plot.
    :param final_df:
    :param genome_length_without_non_coding:
    :param mutation_percentage_synonymous:
    :return:
    ...
    for patient in patients:
        ...
        # Calculate normalized divergence values
        non_syn_data_normalized = calculate_divergence(patient_data, 'non-synonymous', mutation_percentage_non_synonymous,
                                                   genome_length_without_non_coding)

        all_time_points.extend(non_syn_data_normalized.index)
        all_non_syn_values.extend(non_syn_data_normalized.values)
        # Plot data for the current patient
        ax.scatter(non_syn_data_normalized.index, non_syn_data_normalized.values, marker='o', linestyle='-', label=f'{patient}', s=150)

    # Perform linear regression through the origin using scikit-learn
    model = LinearRegression(fit_intercept=False)
    all_time_points = np.array(all_time_points).reshape(-1, 1)
    model.fit(all_time_points, all_non_syn_values)
    slope = model.coef_[0]
    intercept = model.intercept_
    regression_line = slope * np.array(all_time_points)

    # Calculate R^2
    r_squared = model.score(all_time_points, all_non_syn_values)
    ...
    sm_model = sm.OLS(all_non_syn_values, all_time_points_sm).fit()
...
def main():
    ...
    # ->>>>> Else, load the final DataFrame with divergence rates
    ...
    # Different plotting options depending on the analysis you want to perform:

    # 1) Plot divergence rates for synonymous and non-synonymous mutations over time for each patient
    ...
    # 2) Plot divergence rates aggregated by gene
    # plot_divergence_by_gene(final_df)

    # 3) Plot divergence rates aggregated by gene for each patient separately (default gene is 'ORF1ab')
    # plot_divergence_by_gene_per_patient(final_df)

    # 4) Plot divergence rates for non-synonymous mutations over time for all patients in a single scatter plot
    # plot_non_synonymous_divergence_all_patients(final_df, genome_length_without_non_coding,
    #                                         mutation_percentage_non_synonymous)
    # 5) Plot divergence rates for synonymous mutations over time for all patients in a single scatter plot
    # plot_synonymous_divergence_all_patients(final_df, genome_length_without_non_coding, mutation_percentage_synonymous)

    # 6) Plot normalized divergence rates for synonymous and non-synonymous mutations over time per patient in subplots
    plot_normalized_divergence_rates_per_patient_with_regression(final_df, genome_length_without_non_coding,
                                                                 mutation_percentage_non_synonymous,
                                                                 mutation_percentage_synonymous)
    # 7) Plot normalized divergence rates for synonymous and non-synonymous mutations over time per patient in subplots
    # duplicate function with a modification for P5 at timepoint 75 - just for testing
